...
export class AbastecimentoController {
  ...
  @ApiOperation({ 
    ...
    description: 'Cria um abastecimento a partir de uma solicitação de QR Code de veículo usando o código QR code. Esta rota aceita veículos com tipo_abastecimento LIVRE, COM_AUTORIZACAO ou COM_COTA. Preenche automaticamente: veiculoId, motoristaId (se houver vinculado ao veículo ou se informado no body), empresaId (do usuário logado ou se informado no body), solicitanteId, abastecedorId e validadorId (do usuário logado). Valida capacidade_tanque do veículo e CotaOrgao.restante. Atualiza automaticamente CotaOrgao e Processo após criar o abastecimento.'
  })
